#!/usr/bin/env python3
"""Validate a dual-source global-commodities desk packet."""
...
def validate_source_pair(candidate: dict, prefix: str, errors: list[str]) -> set[str]:
    ...
    if not isinstance(pair, list) or len(pair) < 2:
        ...
        return set()
    source_ids, publishers, families, grades = set(), set(), set(), set()
...
def main() -> int:
    ...
    errors = []
    if packet.get("desk") != "global_commodities":
        errors.append("desk must be global_commodities")
